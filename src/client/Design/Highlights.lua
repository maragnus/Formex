...
function Highlights.IsHighlightInstance(instance: Instance?): boolean
	if not instance then return false end
	return selectionHighlight ~= nil and instance == selectionHighlight
end

function Highlights.UpdateSelectionHighlight()
...
function Highlights.UpdateFloorEdgePreview(
	plotInfo: any,
	levelIndex: number,
	points: {Vector2int16},
	isValid: boolean,
...
function Highlights.UpdateWallEdgePreview(plotInfo: any, levelIndex: number, startPoint: Vector2int16, endPoint: Vector2int16, isValid: boolean)
	updateEdgePreview(plotInfo, levelIndex, { startPoint, endPoint }, isValid, false, nil, edgeParts, edgeHighlights, "Wall")
end

function Highlights.UpdateMergeEdgePreview(plotInfo: any, levelIndex: number, segments: {{Start: Vector2int16, End: Vector2int16}}, color: Color3?)
...
function Highlights.ClearFloorEdgePreview()
	clearEdgePreviews(edgeParts, edgeHighlights)
end

function Highlights.UpdateFloorHolePreview(
	plotInfo: any,
	levelIndex: number,
	points: {Vector2int16},
	raiseHeight: number?
...
function Highlights.ClearFloorHolePreview()
	clearEdgePreviews(holeEdgeParts, holeEdgeHighlights)
end

function Highlights.ClearMergeEdgePreview()
	clearEdgePreviews(mergeEdgeParts, mergeEdgeHighlights)
end

function Highlights.UpdateRoomOverlays(plotInfo: any, levelIndex: number)
	if not plotInfo or not plotInfo.PlotData or not plotInfo.PlotData.Rooms then
		Highlights.ClearRoomOverlays()
		return
	end
...
function Highlights.ClearRoomOverlays()
	for floorId, highlight in pairs(roomFloorHighlights) do
		if highlight then
			highlight:Destroy()
		end
